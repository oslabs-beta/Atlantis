...
queryArr.forEach(field => {
  ...
  if(typeof field === "string"){
    ...
    innerNested[fieldName] = jsonObj[fieldName];
    
    if(Array.isArray(jsonObj[fieldName])){
      resultObj = {...resultObj, ...DuplicatedArr(jsonObj[fieldName], field[fieldName])};
      // resultObj[fieldName] = DuplicatedArr(jsonObj[fieldName], field[fieldName]);
    }
  }
})
