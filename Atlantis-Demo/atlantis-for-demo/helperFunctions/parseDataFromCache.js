...
function parseDataFromCache(cacheObj, fieldObj) {
    const result = {};
...
const ObjectFilter = (cacheObj, fieldObj) => {
    let resultObj = {};
    ...
    fieldObj.forEach((field) => {
        ...
        if (typeof field === 'string') {
            ...
            if (cacheObj[fieldName] === null) {
                ...
                resultObj[fieldName] = cacheObj[fieldName].map((obj) => {
                    return ObjectFilter(obj, field[fieldName]);
                });
            }
            else if (typeof cacheObj[fieldName] === 'object') {
                resultObj[fieldName] = ObjectFilter(cacheObj[fieldName], field[fieldName]);
            }
        }
    });
    return resultObj;
